...
def plot_result(wobj, res, config):
    ...
    if config.plot_png:
        ...
        efig.savefig(os.path.join(config.output_dir, eplot_png))
        nfig.savefig(os.path.join(config.output_dir, nplot_png))
        plots.update({'eplot_png': eplot_png,
                      'nplot_png': nplot_png})
    if config.plot_pdf:
        eplot_pdf = '%s.%s.eplot.pdf' % (wobj.name, res.sample_set.name)
        nplot_pdf = '%s.%s.null.pdf' % (wobj.name, res.sample_set.name)
        efig.savefig(os.path.join(config.output_dir, eplot_pdf))
        nfig.savefig(os.path.join(config.output_dir, nplot_pdf))
        plots.update({'eplot_pdf': eplot_pdf,
                      'nplot_pdf': nplot_pdf})
    plt.close()
    return plots
